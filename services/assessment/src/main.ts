/**
 * Punto de entrada del servicio de evaluacion.
 *
...
import { loadAssessmentConfig } from './assessment.module';
...
async function main(): Promise<void> {
  ...
  const app = await bootstrapService({
    ...
    onReady: async (instance) => {
      ...
      // Si NATS esta caido el servicio sigue atendiendo: los eventos se
      // acumulan en la outbox y salen cuando el bus vuelva. Un alumno que
      // entrega su examen no puede quedarse sin entregarlo porque el bus
      // tenga un mal dia.
      try {
