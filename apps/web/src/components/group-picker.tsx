...
export function GroupPicker({
  ...
  if (classmates.length < minCompaneros) {
    return (
      ...
      >
        <p className="font-semibold text-ink-900">Todavía no puedes formar tu grupo</p>
        <p className="mt-1 text-ink-700">
          {takenCount > 0
            ? `Esta actividad se hace en grupo, y los compañeros que quedan libres no llegan al mínimo: ${takenCount} ya empezaron con otro grupo.`
            : 'Esta actividad se hace en grupo y no hay suficientes compañeros en tu salón todavía.'}{' '}
  ...
  return (
    ...
    >
      ...
      <div>
        <h2 className="font-display text-lg font-semibold">Esta actividad se hace en grupo</h2>
        <p className="mt-1 text-sm text-ink-500">
          {minSize === maxSize
            ? `Sois ${minSize}, contándote a ti.`
            : `Sois de ${minSize} a ${maxSize}, contándote a ti.`}{' '}
          La nota es la misma para todo el grupo.
        </p>
      </div>
      ...
      <details data-group="picker" className="rounded-lg border border-line-200 bg-surface-50">
        <summary className="cursor-pointer px-4 py-3 text-sm font-medium text-ink-700">
          Elegir a mis compañeros
          <span className="ml-1 font-normal text-ink-500">
            ({classmates.length} disponibles)
          </span>
        </summary>

        <fieldset className="grid gap-1 px-4 pb-4">
          <legend className="sr-only">Compañeros de tu grupo</legend>
          ...
          <p className="border-b border-line-200 py-2 text-sm text-ink-500">
            {studentName} <span className="text-ink-400">— tú</span>
          </p>
          ...
          {classmates.map((classmate) => (
            ...
            >
              ...
              />
              <span className="text-ink-900">{classmate.fullName ?? 'Un compañero'}</span>
            </label>
          ))}
        </fieldset>
      </details>
      ...
      <p className="text-xs text-ink-500">
        Puedes elegir {minCompaneros === maxCompaneros
          ? `${minCompaneros}`
          : `entre ${minCompaneros} y ${maxCompaneros}`}{' '}
        {maxCompaneros === 1 ? 'compañero' : 'compañeros'}.
        {takenCount > 0
          ? ` No salen ${takenCount} que ya empezaron con otro grupo.`
          : ''}
      </p>
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      <button type="submit" disabled={pending} className="btn btn-primary">
        ...
        {pending ? 'Abriendo la actividad…' : 'Empezar con mi grupo'}
      </button>
      <p className="text-sm text-ink-500">
        En cuanto empecéis, vuestros nombres dejan de salirles a los demás.
      </p>
    </div>
  );
}
